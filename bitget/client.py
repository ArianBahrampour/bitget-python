...
class Client(object):
    ...
    def _request(self, method, request_path, params, cursor=False):
        if method == c.GET:
            request_path = request_path + hash.parse_params_to_str(params)
        # url
        ...
        sign = hash.sign(hash.pre_hash(timestamp, method, request_path, str(body)), self.API_SECRET_KEY)
        header = hash.get_header(self.API_KEY, sign, timestamp, self.PASSPHRASE)
